...
/*
 * Procbased Primary Controls capabilities
 * See SDM volume 3C, section 24.6.2
 */
 struct capability_info procbasedPrimary[21] = 
 	{ 
...
/*
 * Procbased Secondary Controls capabilities
 * See SDM volume 3, section 24.6.2
 */
struct capability_info procbasedSecondary[27] = 
	{ 
...
detect_vmx_features(void)
{
	...
	pr_info("Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(entryCTLS, 12, lo, hi);
	
	/* Procbased Primary controls */
	rdmsr(IA32_VMX_PROCBASED_CTLS, lo, hi);
	pr_info("Procbased Primary Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbasedPrimary, 21, lo, hi);
